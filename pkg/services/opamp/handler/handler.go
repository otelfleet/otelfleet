...
var _ services_int.OpAmpServerHandler = (*CollectorHandler)(nil)
...
func (s *CollectorHandler) persistAgentInformation(ctx context.Context, msg *protobufs.AgentToServer) error {
...
func (s *CollectorHandler) bootstrap(
	ctx context.Context,
	logger *slog.Logger,
	instanceUID string,
	...
	if err != nil {
		...
		return opamputil.ErrorResponse(message.InstanceUid, opamputil.NewUnavailableError("failed to verify agent is registered"))
	}
	if !ok {
...
func (s *CollectorHandler) updateConnectionState(ctx context.Context, msg *protobufs.AgentToServer) bool {
...
func (s *CollectorHandler) handleRemoteConfigStatus(
	ctx context.Context,
...
// checkForConfigUpdate returns the config to push when the agent is not already
// running the config the server currently resolves for it.
func (s *CollectorHandler) checkForConfigUpdate(ctx context.Context, agentID string, msg *protobufs.AgentToServer) *protobufs.AgentRemoteConfig {
	logger := logutil.FromContext(ctx)
...
func (s *CollectorHandler) appliedConfigHash(ctx context.Context, msg *protobufs.AgentToServer) []byte {
...
func (s *CollectorHandler) calculateHash(agentToConfigMap *protobufs.AgentConfigMap) []byte {
...
func (s *CollectorHandler) constructConfig(ctx context.Context) (*resolvedConfig, error) {
	...
	if grpcutil.IsError(codes.NotFound, err) {
		...
		logger.With("err", err).Error("failed to construct a config from filters and agent labels")
		return nil, err
	}
	if config.GetRaw() == nil {
		panic("bug/todo : config is built using references")
	}
	logger.With("config", configRef).Info("agent has an assigned config from control plane")
	return &resolvedConfig{
		configMap: util.ProtoConfigToAgentConfigMap(config.GetRaw()),
	}, nil
}

// recordAssignment persists what the server resolved for the agent so config
// sync status can be computed against it.
func (s *CollectorHandler) recordAssignment(ctx context.Context, agentID string, resolved *resolvedConfig, hash []byte) {
	// TODO : this would be used for approving config updates
	panic("implement me")
	// logger := logutil.FromContext(ctx)
