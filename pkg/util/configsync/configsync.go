...
func ComputeConfigSyncStatus(
...
) (v1alpha1.ConfigSyncStatus, string) {
	// If no assigned hash, we can't determine sync status
	if len(assignedHash) == 0 {
		return v1alpha1.ConfigSyncStatus_CONFIG_SYNC_STATUS_UNKNOWN, "no assigned config"
	}

	remoteStatus, err := remoteStatusStore.Get(ctx, agentID)
	if err != nil {
		return v1alpha1.ConfigSyncStatus_CONFIG_SYNC_STATUS_OUT_OF_SYNC, "no status reported"
	}

	// Check if the hash matches what we assigned
	if !bytes.Equal(remoteStatus.GetLastRemoteConfigHash(), assignedHash) {
		return v1alpha1.ConfigSyncStatus_CONFIG_SYNC_STATUS_OUT_OF_SYNC, "hash mismatch"
	}
	...
	case protobufs.RemoteConfigStatuses_RemoteConfigStatuses_FAILED:
		return v1alpha1.ConfigSyncStatus_CONFIG_SYNC_STATUS_ERROR, remoteStatus.GetErrorMessage()
	default:
		return v1alpha1.ConfigSyncStatus_CONFIG_SYNC_STATUS_OUT_OF_SYNC, "unknown status"
	}
}
